...
Usage:
    ...
    python -m app.cli sync-plans   # create Paystack plans from the catalog

The seed command is idempotent — safe to re-run after every deploy.
Production deploys should run it as a release hook.
"""
...
def cmd_seed_info(_: argparse.Namespace) -> int:
    ...
    if MAPPINGS_FILE.exists():
        ...
        # Count by source framework
        by_source: dict[str, int] = {}
        for m in mappings.mappings:
            by_source[m.source_framework.value] = (
                by_source.get(m.source_framework.value, 0) + 1
            )
...
def cmd_sync_plans(args: argparse.Namespace) -> int:
    """Create Paystack plans from the catalog and print env-var setup lines.

    Idempotent — Paystack returns the existing plan if a plan with the same
    name already exists, so re-runs are safe.

    Usage:
        # Test mode (uses USE_MOCK_PAYMENTS to dry-run; prints fake codes)
        python -m app.cli sync-plans --dry-run

        # Live (or test-mode Paystack)
        PAYSTACK_SECRET_KEY=sk_test_xxx python -m app.cli sync-plans

    The output is shell-format env vars suitable for piping into a deploy
    config: `python -m app.cli sync-plans >> .env.production`
    """
    ...
    try:
        ...
        print(
            "Hint: set PAYSTACK_SECRET_KEY or pass --dry-run to test against the mock.",
            file=sys.stderr,
        )
        return 1

    print(
        f"# Syncing {len(CATALOG)} plans to Paystack "
    ...
    for plan in CATALOG:
        try:
            ...
            print(
                f"# FAILED {plan.lookup_key}: {exc}",
                file=sys.stderr,
            )
            continue
        env_var = _env_var_for_plan(plan)
        env_lines.append(f"{env_var}={ps_plan.plan_code}")
        print(
            f"# {plan.lookup_key:30s} {plan.amount_major:>12,.2f} "
            f"{plan.currency.value} -> {ps_plan.plan_code}",
            file=sys.stderr,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    sync_parser = sub.add_parser(
        "sync-plans", help="Create Paystack plans from the catalog"
    )
    sync_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Use the mock Paystack client; produce fake plan codes",
    )
